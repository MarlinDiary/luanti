...
[策略示例说明](student-strategies.md) 分开提供一份有界入门策略和一份默认持续运行的自主矿工。二者都只组合本页公开技能；采集保持 `allow_tool_crafting=False`，合成保持 `gather=False`，因此工具升级、下挖深度、矿道方向、失败反馈和返程仍然是可见的学生策略，而不是 SDK 暗中完成。
